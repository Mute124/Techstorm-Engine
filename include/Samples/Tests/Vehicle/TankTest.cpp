...
void TankTest::Initialize()
{
	...
	const float barrel_radius = 0.7f;
...
void TankTest::ProcessInput(const ProcessInputParams &inParams)
{
	...
	mFire = inParams.mKeyboard->IsKeyPressed(DIK_F);
}
...
void TankTest::PrePhysicsUpdate(const PreUpdateParams &inParams)
{
	...
	const float bullet_mass = 7.0f; // Normal projectile weight is around 7 kg, use an increased value so the momentum is more realistic (with the lower exit velocity)
